...
const resolvers = {
  ...
  Mutation: {
    ...
    addGoal: async (
      parent,
      { goalTitle, category, frequency, flowerType, endDate },
      context
    ) => {
      if (context.user) {
    ...
    addActivity: async (parent, { goalId, activityText }, context) => {
      if (context.user) {
        return Goal.findOneAndUpdate(
          ...
          {
            $addToSet: {
              activities: { activityText, username: context.user.username },
            },
          },
          {
            new: true,
    ...
    removeActivity: async (parent, { goalId, activityId }, context) => {
      if (user.context) {
        return Goal.findOneAndUpdate(
          ...
          {
            $pull: {
              activities: { _id: activityId, username: context.user.username },
            },
          },
          { new: true }
        );
